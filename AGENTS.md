...
> 当前阶段：**phase1-implementation / P1.5 语义整理** —— SSE 流式与 Turn / Context / Session 编排链路已跑通。
...
│   │   │   └── memory/                 # InMemoryConversationStore（将来 postgres/ 同级）
│   │   ├── agent/
...
1. `handoff.md`
2. `docs/index.md`
3. 按任务需要挑读：ADR 09（当前语义与包结构）、ADR 07（P1 底座决策）、`docs/CONTEXT.md`

## 文档权威层级

- 需求以《项目需求说明书》为核心参考；技术选型 / 架构结论以 `docs/decisions/` 最新决策为准。
- **ADR 09 修订 ADR 08**：根包改为 `com.openalice`，`port + memory` 收敛为 `repository`，并引入 Turn / AgentEvent / ContextAssembler / SessionCoordinator。
...
- 包依赖单向：`model ← repository(+memory)`、`model ← agent`、`service → repository + agent + model`、`controller → service + dto`、`config` 组装 repository 与 agent。
...
- `controller` 只做 HTTP/SSE 翻译，业务写在 `service`；换存储实现只动 `config`（组合根）。
- 测试分层：`model` 纯单元测试；`repository.memory` 存储测试；`service` 用 fake `AgentRuntime` 测编排；`agent` 用默认 mock 模型测 runtime；`controller` 做 SpringBoot 集成测试。
