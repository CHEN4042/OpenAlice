...
| 版本 | v0.3 |
| 日期 | 2026-09-07 |
| 配套代码 | 单模块收敛 + enums 集中与命名统一后（本地未推送） |
| 读者 | 项目唯一开发者本人（边做边学） |
...
├── enums/      纯枚举集中地：MessageRole / LlmProvider（零依赖，谁都可用）
...
| enums | `.../enums/MessageRole.java` | 说话者角色：USER / ASSISTANT / SYSTEM | ★ |
| enums | `.../enums/LlmProvider.java` | 模型来源枚举：auto / mock / deepseek / agentrouter（zero-dep，随带默认地址/模型/key 环境变量名） | ★★ |
...
| controller | `.../controller/ChatController.java` | HTTP 入口：只做翻译，转给 ChatService | ★★ |
...
### 4.1 HTTP 层：`ChatController`（入口翻译官）
...
- `LlmProvider` 枚举**不在这里**：它零依赖，已集中到顶层 `enums/`（§5.2 详解）；
...
- 正常返回：`ChatResponse` record → Spring/Jackson 自动转 JSON；
...
完整链条：**curl → ChatController（翻译）→ ChatService（编排：先存 user → 问 agent → 再存 assistant）→ AgentRuntime（瘦身执行器）→ HarnessAgent → LlmModelFactory 选模型 → 模型返回 → 回复写进 MemoryPort → 返回 JSON**。
...
- 类里定义了 4 个 `OPENALICE_*` 环境变量名常量（PROVIDER / MODEL / BASE_URL / PROXY）；
- `fromEnvironment()`：把环境变量读进来。**注意：key 不在这里**——两个 key 的环境变量名
  （`OPENALICE_DEEPSEEK_API_KEY` / `OPENALICE_AGENTROUTER_API_KEY`）跟着 `LlmProvider`
  枚举走（§5.2），这里只放 provider / model / baseUrl / proxy。

### 5.2 枚举：`LlmProvider`（文件：`src/main/java/openalice/enums/LlmProvider.java`）

> 本次微调把它从 `agent/llm/` 移到了顶层 `enums/`（连同 `MessageRole`）。原因：它是**纯数据**，
> 谁都能用；不依赖 agent 域，放 `agent/llm` 会让 `agent` 反向背上「全局枚举」的包袱。
...
| `src/test/java/openalice/controller/ChatControllerTest` | 不真开端口，用 MockMvc 打完整链路 | Controller 的翻译职责 + 参数校验 |
...
| v0.3 | 2026-09-07 | enums/命名统一（本地未推送） | `MessageRole`/`LlmProvider` 集中到顶层 `enums/`（零依赖，key 环境变量名归枚举）；`ChatApiController → ChatController`、`ChatReply → ChatResponse`（与 ChatRequest 配对）；全文类名/路径同步 |
