...
| 版本 | v0.6 |
| 日期 | 2026-09-08 |
| 配套代码 | P1.5 语义重构（已 commit）+ ADR 10 业务包结构整理（本地未 commit） |
| 读者 | 项目唯一开发者本人（边做边学） |
...
├── model/                      # 共享词汇：纯模型与值对象（跨 chat / agent 上提）
│   ├── ChatMessage.java
│   ├── UserId.java
│   ├── SessionId.java
│   ├── MessageRole.java
...
当前测试共 **22 个**，全部通过（Java 21 环境标准 `mvn clean test`）。
...
| `chat/service/ContextAssemblerTest` | 显式上下文包含当前 USER、窗口读取。 |
