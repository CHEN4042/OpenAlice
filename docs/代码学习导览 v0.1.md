# 代码学习导览（陪你读懂 OpenAlice 的代码）

> 这份文档是**地图 + 教材 + 练习册**，不是代码百科全书。
> 目标：让你能合上文档，把「一次 `你好` 从 HTTP 到 AI 回复再到记忆」讲清楚，并且敢动手改代码。
> 用法：先读 §1 学习方法 → §2/§3 看地图 → §4 跟着主线走一遍 → §5 看 provider 专题 → §7/§8 自测与练习。

| 项目 | 内容 |
| :--- | :--- |
| 版本 | v0.9（M1 PostgreSQL 会话持久化已落地；部分历史章节仍保留演进注记） |
| 日期 | 2026-09-20 |
| 配套代码 | HEAD `7e445bc` + M1 工作区（PostgresConversationStore + Flyway） |
| 读者 | 项目唯一开发者本人（边做边学） |
| 配套文档 | `docs/index.md`、`docs/CONTEXT.md`、`handoff.md`、ADR 10 / ADR 09 |

---

---

> ⚠️ **演进提示**：本文部分历史章节保留了旧类名作为学习线索；当前代码以 `handoff.md`、ADR 10/13/14 和源码为准。
> `agent/runtime/` 已拆除，`AgentRuntime` → `agent.AgentExecutor` + `agent.AgentScopeReActAgent`；
> `agent.llm` 上提为顶层 `com.openalice.llm/`，mock 已删除；会话存储新增
> `chat.store.postgres.PostgresConversationStore` + Flyway `session_message` 迁移。

## 0. 先回答三个问题（自检，答案在文末对应小节）

1. 一次「你好」从 `curl` 发出到收到 SSE 回复，经过了哪几个角色？（§4.7）
2. 为什么请求里没有 `userId`，存储里却仍有 `userId`？（§4.1 / §4.5）
3. 同一个 session 同时发两条请求时，系统怎么避免历史和回复串线？（§4.3）

答不出来不丢人——这份文档就是为此写的。读完再回来答一次。

---

## 1. 怎么学 AI 写的代码

### 1.1 三条学习纪律

1. **我讲给你听**：先说「它解决什么问题 → 为什么这么设计 → 对应哪个文件」，再给细节。
2. **你回讲给我听**：看完一段代码，用自己的话复述；讲不清的地方就是下一步要学的点。
3. **你亲手改一处**：任何新代码，至少改动一处并跑通一次（哪怕只是改系统提示词），禁止只看不改。

### 1.2 为什么强调「亲手改」

参考外部工程学习方法的共同结论是：**跑起来 → 找入口 → 追一条真实请求到底 → 写测试验证理解 → 故意改坏看失败模式**。直接接受 AI 输出、不看 diff、坏了乱试，容易形成 vibe coding 陷阱。本仓库用 §7 的测试地图和 §9 的命令速查落实这套流程。

---

## 2. 项目全景：先看地图，再上路

### 2.1 P1.5 + ADR 10 物理结构（ADR 13 修订 model 内容）

源码根包已从 `openalice` 改为 `com.openalice`。P1.5 把「业务消息」「会话存储」「Agent 输入输出」「HTTP 翻译」放在更符合职责的位置；ADR 10 又参考 Jarvis `pet/enums` 形态，把顶层从「按层分」调整为「按业务/内核/共享分」，业务包内部再按类型整理：

```text
src/main/java/com/openalice/
├── OpenAliceApplication.java
├── model/                      # 共享词汇：ChatMessage / MessageRole
├── chat/                       # ★ 业务：对话
│   ├── service/                # ChatService / ContextAssembler / SessionCoordinator
│   └── store/
│       ├── ConversationStore.java
│       ├── StoredMessage.java
│       ├── memory/             # 测试 / 显式降级实现
│       └── postgres/           # 默认实现：PostgresConversationStore
├── agent/                      # AgentExecutor / AgentScopeReActAgent / tool / events
├── llm/                        # top-level provider / model factory / HTTP transport
├── controller/                 # HTTP/SSE 翻译
├── dto/                        # API 出入参
└── config/                     # Spring 组合根
```

测试镜像同样放在 `src/test/java/com/openalice/`。

### 2.2 结构调整的理由

| 变化 | 原因 |
| :--- | :--- |
| `openalice` → `com.openalice` | 符合常规 Java 命名空间，避免裸顶级包；测试与生产包结构保持镜像。 |
| `MemoryPort` → `ConversationStore` | 它的承诺只是「会话消息存储」，不是所有记忆能力；名字收窄后，未来 PG 实现也更自然。 |
| 顶层分层 → 业务/内核/共享（ADR 10） | 参考 Jarvis `pet/enums` 业务自治形态：`chat` 业务包 + `agent` 内核 + 共享 `model`，包内再按类型子包整理。 |
| `repository/` → `chat/store/` | 会话存储属于「对话」业务：收进 `chat.store`；实现放 `chat.store.memory`，未来 PG 放 `chat.store.postgres`。 |
| `MessageRole` 移入 `model` | 它是消息模型的一部分，不是全局杂项枚举。 |
| `LlmProvider` 移入 `agent.llm` | 它只被模型接入层使用，不应放在全局 `enums` 包里。 |
| 删除顶层 `enums/` | 避免成为没有边界的杂货包。 |

### 2.3 依赖方向（防乱）

```text
model ← chat.store
model ← agent
chat.service → model + chat.store + agent
controller → chat.service
config 组装 chat.store + agent
```

重点红线：

- `model` 只放纯 POJO / 值对象，不放 Spring、数据库、AgentScope 注解或依赖；
- `agent` 不读 `ConversationStore`；上下文由 service 组装成 `AgentRequest` 后显式传入；
- `controller` 只做 HTTP / SSE 翻译，不写业务；
- 换存储实现时，只动 `config` 组合根。

---

## 3. 一张图看懂调用关系

```mermaid
flowchart LR
    U[curl / 浏览器] -->|POST /api/v1/chat| C[ChatController]
    C -->|ChatRequest| S[ChatService]
    S -->|单 turn 加锁| K[SessionCoordinator]
    K -->|串行执行| A[ContextAssembler]
    A -->|history 最近 N 条| M[(ConversationStore)]
    A -->|AgentRequest| R[AgentRuntime]
    R -->|clearContext + 显式 messages| H[HarnessAgent]
    H -->|选择模型| F[LlmModelFactory]
    F -->|mock / DeepSeek / AgentRouter| L[模型]
    R -->|AgentEvent| S
    S -->|done 时写 ASSISTANT| M
    S -->|AgentEvent| C
    C -->|SSE| U
```

读图顺序：**入口 → 编排 → session 并发控制 → 上下文组装 → Agent 执行 → 存储 → SSE 出口**。

---

## 4. 主线：一次 `POST /api/v1/chat` 的完整旅程

### 4.1 HTTP 层：`ChatController`

文件：`src/main/java/com/openalice/controller/ChatController.java`

```java
@PostMapping(value = "/chat", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public Flux<ServerSentEvent<ChatStreamEvent>> chat(@RequestBody ChatRequest request) {
    return chatService.stream(request)
            .map(ChatController::toStreamEvent)
            .map(event -> ServerSentEvent.<ChatStreamEvent>builder()
                    .event(event.type())
                    .data(event)
                    .build());
}
```

看懂要点：

- `ChatRequest` 只包含 `sessionId` 和 `message`；
- Controller 不认识 Agent、存储、ContextAssembler，只把 `AgentEvent` 翻译成 `ChatStreamEvent` / SSE；
- 历史 API 是 `GET /api/v1/sessions/{sessionId}/messages`，同样不暴露 userId。

### 4.2 编排层：`ChatService`

文件：`src/main/java/com/openalice/chat/service/ChatService.java`

一轮对话的生命周期（ADR 13 起不再有进程内 Turn 状态机）：

```text
构造 StoredMessage（USER，默认用户）
  ↓ SessionCoordinator 内写 USER
  ↓ ContextAssembler 组装 AgentRequest（读最近 N 条）
  ↓ AgentRuntime.stream()
  ↓ DoneEvent 时写入 ASSISTANT
（异常兜底转 ErrorEvent）
```

`ChatService` 负责单 turn：

1. 校验 sessionId（非空）；
2. 构造 `StoredMessage.user(...)`（默认用户）写入存储；
3. 在 session 锁内组装上下文、调用 Agent；
4. 收到 `DoneEvent` 后写 `StoredMessage.assistant(...)`；异常兜底转 `ErrorEvent`。

它不是简单「转发」，而是这轮对话的**总指挥**。存储行 `StoredMessage` 承担 id / userId / sessionId / 时间戳，消息本体 `ChatMessage` 只表达 role / content。

### 4.3 并发层：`SessionCoordinator`

文件：`src/main/java/com/openalice/chat/service/SessionCoordinator.java`

设计承诺：

- 同一个 `sessionId`：完整 turn 串行执行；
- 不同 `sessionId`：可并行执行；
- 锁的范围包括 USER 写入、历史读取、Agent 调用、ASSISTANT 写入，不只在方法入口加一下锁。

实现要点是每个 session 一个 `Semaphore(1)`，用 `Flux.usingWhen` 保证完成、错误、取消时都会释放。当前保留 session 对象而不做复杂清理，这是小规模单用户场景下的保守选择；未来 session 数量极大时再评估安全的清理机制，不要为了清理引入竞态。

### 4.4 上下文组装：`ContextAssembler`

文件：`src/main/java/com/openalice/chat/service/ContextAssembler.java`

```java
List<StoredMessage> stored = conversationStore.history(sessionId, contextWindowSize);
List<ChatMessage> context = stored.stream().map(StoredMessage::message).toList();
```

`assemble(userId, sessionId)` 做的事：

1. 从 `ConversationStore` 读最近 `contextWindowSize` 条存储行；
2. 校验上下文最后一条是当前 USER 消息（刚写入，串行锁内保证）；
3. 把存储行映射为纯 `ChatMessage` 列表；
4. 携带系统提示词，生成不可变的 `AgentRequest`。

配置项：

```yaml
openalice:
  agent:
    context-window-size: 20
```

这个类是「业务历史」和「Agent 输入」之间的显式翻译层。它让 Agent 适配器不必读存储，也让上下文策略以后可以独立演进。

### 4.5 存储层：`ConversationStore`

文件：

- `src/main/java/com/openalice/chat/store/ConversationStore.java`
- `src/main/java/com/openalice/chat/store/StoredMessage.java`
- `src/main/java/com/openalice/chat/store/postgres/PostgresConversationStore.java`（默认）
- `src/main/java/com/openalice/chat/store/memory/InMemoryConversationStore.java`（测试 / 显式降级）

接口只有两个能力：

```java
public interface ConversationStore {
    void append(StoredMessage message);
    List<StoredMessage> history(String sessionId, int limit);
}
```

为什么外部 API 没有 userId，存储里却有：

- 产品当前是单用户，HTTP 协议不应该让调用方传身份；
- 服务端固定默认用户（`ChatService` 私有常量）；
- 消息本体 `ChatMessage` 只含 role / content；`StoredMessage` 才是存储行——`id` / `userId` / `sessionId` / `createdAt` 一眼可读（ADR 13：归属与时间戳下沉存储侧），继续保留 `userId` 是为了未来加认证时从上下文注入真实用户，不推翻存储模型。

默认实现是 PostgreSQL：每条消息写入 `session_message`，Flyway 在启动时创建表和索引；进程重启后历史仍可恢复。`InMemoryConversationStore` 只用于测试或显式降级，service / agent 不感知具体实现。

### 4.6 Agent 层：`AgentRequest` + `AgentEvent`

文件：

- `src/main/java/com/openalice/agent/AgentRequest.java`
- `src/main/java/com/openalice/agent/AgentEvent.java`
- `src/main/java/com/openalice/agent/TextDeltaEvent.java`
- `src/main/java/com/openalice/agent/DoneEvent.java`
- `src/main/java/com/openalice/agent/ErrorEvent.java`
- `src/main/java/com/openalice/agent/runtime/AgentRuntime.java`

接口：

```java
Flux<AgentEvent> stream(AgentRequest request);
```

`AgentRequest` 显式包含：

- `userId` / `sessionId`（构造 RuntimeContext；单用户下 userId 为服务端默认）
- `conversationContext`（最近业务历史 + 当前 USER，纯 `ChatMessage`）
- `systemPrompt`

`AgentEvent` 是 sealed interface，只有三种实现：

| 事件 | 含义 |
| :--- | :--- |
| `TextDeltaEvent` | 一段增量文本 |
| `DoneEvent` | 本轮完成，带完整回复 |
| `ErrorEvent` | 本轮失败，带错误信息 |

`AgentScopeAgentRuntime` 每次调用前会 `clearContext(context)`，再把 `AgentRequest.conversationContext` 转成 AgentScope messages。这样 `ConversationStore` 是业务历史唯一真相源，AgentScope state store 只是运行态 scratch，不会形成双份历史。

> AgentScope 2.0.2 的 hook 不允许把 SYSTEM message 直接注入 input messages；系统提示词仍由 `HarnessAgent.builder().sysPrompt(...)` 设置。`AgentRequest.systemPrompt` 和 `AgentRuntimeProperties.systemPrompt()` 来自同一配置 Bean，当前保持一致。

### 4.7 出口与错误处理

- 正常聊天：若干 `text_delta` → `done`；
- Agent / provider 异常：转换为 `error` 事件；已写入的 USER 消息保留，未完成 ASSISTANT 不写入；
- 参数错误：`ApiExceptionHandler` 统一转 HTTP 400；
- 可用端点：
  - `GET /actuator/health`
  - `POST /api/v1/chat`
  - `GET /api/v1/sessions/{sessionId}/messages`

完整链条：

```text
curl -N
→ ChatController
→ ChatService
→ SessionCoordinator
→ ContextAssembler
→ ConversationStore
→ AgentRequest
→ AgentRuntime
→ HarnessAgent
→ Flux<AgentEvent>
→ ChatController
→ SSE
```

---

## 5. 专题：LLM 双 provider 是怎么接进去的

### 5.1 配置

文件：`src/main/java/com/openalice/config/OpenAliceSettings.java`（`openalice.*` 配置绑定）

环境变量：

| 变量 | 说明 |
| :--- | :--- |
| `OPENALICE_LLM_PROVIDER` | `auto` / `deepseek` / `agentrouter` |
| `OPENALICE_AGENTROUTER_API_KEY` | AgentRouter 中转站 key（优先） |
| `OPENALICE_DEEPSEEK_API_KEY` | DeepSeek 官方 key（兜底） |
| `OPENALICE_LLM_MODEL` | 模型名覆盖 |
| `OPENALICE_LLM_BASE_URL` | Base URL 覆盖 |
| `OPENALICE_LLM_PROXY` | HTTP 代理 |
| `OPENALICE_SYSTEM_PROMPT` | 系统提示词覆盖 |

### 5.2 Provider 与工厂

文件：

- `src/main/java/com/openalice/llm/LlmProvider.java`
- `src/main/java/com/openalice/llm/LlmModelFactory.java`
- `src/main/java/com/openalice/llm/ConfiguredHttpTransport.java`

auto 回退链：

```text
AgentRouter 有 key → AgentRouter
DeepSeek 有 key   → DeepSeek
都没有 key        → 启动 / 调用失败（不回退 mock）
```

`ConfiguredHttpTransport` 负责代理和 AgentRouter WAF 需要的请求头指纹。key 只从环境变量读取，不写入代码或配置文件。

### 5.3 改人设的位置

想改 Alice 人设，改 `src/main/resources/application.yml` 的 `openalice.agent.system-prompt`，或用环境变量覆盖。角色契约与异议协议见 `_bmad-output/specs/spec-openalice/persona-contract.md`；当前不要提前新建 `persona/` 文件加载层（ADR 14）。

---

## 6. 从 Jarvis 借了什么，没抄什么

参考本地 Jarvis 后，P1.5 吸收了三类模式：

| Jarvis 的做法 | OpenAlice 的落地 |
| :--- | :--- |
| Controller 尽量薄，业务编排进 service | `ChatController` 只翻译 HTTP / SSE，业务归 `ChatService`。 |
| 用户身份不散落在请求处理里 | HTTP 不收 `userId`，服务端固定默认用户（`ChatService` 私有常量）；存储行保留 `userId` 等未来认证。 |
| Agent 调用前显式组装上下文 | `ContextAssembler` 从 `ConversationStore` 读最近 N 条并生成 `AgentRequest`。 |
| 业务包自治 + 包内类型整理（`pet/enums` 形态） | `chat` 业务包内含 `service/`、`store/`；共享词汇上提 `model/`（ADR 10）。 |

没有照搬的点：

- 不照搬平铺大包；OpenAlice 顶层保留 `model / agent / chat(service|store) / controller / dto / config` 的清晰边界（ADR 10）；
- 不把 userId 作为外部路由键；
- 不让 Agent 直接读业务存储。

---

## 7. 测试在教我们什么

当前测试共 **25 个**，全绿（Java 21）。

| 测试文件 | 验证什么 |
| :--- | :--- |
| `model/ChatMessageTest` | role / content 构造、blank 内容与 null role 校验。 |
| `chat/store/memory/InMemoryConversationStoreTest` | 内存实现：会话隔离、最近 N 条、不可变快照、并发写入。 |
| `chat/store/postgres/PostgresConversationStoreTest` | PostgreSQL SQL 契约：跨实例恢复、会话隔离、最近 N 条顺序、limit 边界。 |
| `chat/service/ChatServiceTest` | 编排顺序、流式事件、ASSISTANT 持久化、错误路径。 |
| `chat/service/ContextAssemblerTest` | 显式上下文窗口读取、必须以 USER 结尾校验。 |
| `chat/service/SessionCoordinatorTest` | 同 session 串行、不同 session 可并行。 |
| `agent/AgentScopeReActAgentTest` | AgentScope ReAct 流式事件、完成回复、错误路径与多 session 隔离。 |
| `controller/ChatControllerTest` | HTTP / SSE 协议、完整事件序列、历史接口。 |

练习：把 `ChatService.stream()` 中 `conversationStore.append(userMessage)` 注释掉，跑 `mvn test`，观察 `ContextAssemblerTest` / `ChatServiceTest` 哪个先红，再改回来。这就是「故意改坏，理解失败模式」。

---

## 8. 学习追踪表（每次会话更新）

> 状态：✅ 能讲清 ｜ 🟡 半懂 ｜ ❌ 卡住。

| 主题 | 对应位置 | 状态 | 日期 | ONE thing 笔记 |
| :--- | :--- | :--- | :--- | :--- |
| com.openalice 包结构 | §2.1 | | | |
| ConversationStore 与旧 MemoryPort 的区别 | §2.2 / §4.5 | | | |
| 为什么 HTTP 不收 userId | §4.1 / §4.5 | | | |
| Turn 状态机 | §4.2 | | | |
| SessionCoordinator 的锁范围 | §4.3 | | | |
| ContextAssembler 显式组装上下文 | §4.4 | | | |
| AgentRequest / AgentEvent | §4.6 | | | |
| AgentScope scratch 与业务历史分离 | §4.6 | | | |
| SSE 事件映射 | §4.1 / §4.7 | | | |
| 双 provider 回退链 | §5.2 | | | |
| 怎么故意改坏让测试变红 | §7 | | | |

---

## 9. 常用命令速查

```bash
mvn clean test          # Java 21 环境的标准验证
mvn package             # 打包
mvn spring-boot:run     # 启动
curl http://localhost:8080/actuator/health

curl -N -X POST http://localhost:8080/api/v1/chat \
  -H 'Accept: text/event-stream' \
  -H 'Content-Type: application/json' \
  -d '{"sessionId":"default","message":"你好"}'

curl http://localhost:8080/api/v1/sessions/default/messages
```

IntelliJ 跳转路线：

```text
ChatController.chat
  → ChatService.stream
    → SessionCoordinator.serialize
    → ContextAssembler.assemble
      → ConversationStore.history
    → AgentRuntime.stream
      → AgentScopeAgentRuntime.stream
        → HarnessAgent.streamEvents
```

---

## 10. 维护规则

1. 任何包结构、接口、端点、协议、测试结构变化，都要同步更新本文件；
2. 结构性更新递增版本号，并在「变更记录」追加一行；
3. 同步检查 `docs/index.md`、`handoff.md`、`AGENTS.md` 中的目录与 API 摘要；
4. 讲解时保持 What / Why / 对应文件的结构；
5. 不提前创建空包、空模块和无人引用的死代码。

## 变更记录

| 版本 | 日期 | 配套状态 | 变更 |
| :--- | :--- | :--- | :--- |
| v0.8 | 2026-09-08 | ADR 13 消息拆分 | `ChatMessage` 只剩 role/content；新增 `chat.store.StoredMessage`（id/归属/时间戳）；`history(sessionId, limit)`；删 `ConversationTurn`/`TurnStatus`/blocking 便利层；目录树与 4.1/4.2/4.4/4.5/4.6/6/7 同步。 |
| v0.1 | 2026-09-07 | 双 provider | 初版：全景地图 + `/chat` 主线 + provider 专题 + 学习追踪表。 |
| v0.2 | 2026-09-07 | 单模块收敛 | 新增 service 编排层说明；全部路径同步单模块结构。 |
| v0.3 | 2026-09-07 | enums 整理 | `MessageRole` / `LlmProvider` 集中到顶层 `enums/`。 |
| v0.4 | 2026-09-07 | SSE 流式 | `/api/v1/chat` 改为 `text/event-stream`，补充事件流链路。 |
| v0.5 | 2026-09-08 | P1.5 重构 | 根包改为 `com.openalice`；`MemoryPort` → `ConversationStore`；单用户 API 去 userId；新增 Turn、ContextAssembler、AgentEvent、SessionCoordinator；物理结构与测试地图全面同步。 |
| v0.6 | 2026-09-08 | ADR 10 业务包 | 顶层改为业务/内核/共享：`repository` → `chat/store`、`service` → `chat/service`；全文目录与路径同步；测试地图同步。 |
| v0.7 | 2026-09-08 | ADR 12 id String 化 | 删除 `UserId` / `SessionId` 值对象，model 只剩 4 个文件；`history(String, String, int)`；`UserId.DEFAULT` → `ChatMessage.DEFAULT_USER_ID`；目录树与 4.2/4.5 同步。 |
