...
| 项目 | 内容 |
| :--- | :--- |
| 版本 | v0.8（§2~§7 结构/命名章节滞后于 HEAD，见 §0 红字提示） |
| 日期 | 2026-09-09 |
| 配套代码 | HEAD `3aa198c`（ADR 11/12/13 已合入）+ 工作区 Agent 执行层重构（AgentExecutor + ReAct，未 commit） |
| 读者 | 项目唯一开发者本人（边做边学） |
...
> ⚠️ **§0.1 结构已演进（本节之后的目录树/类名可能过时）**：本文件正文仍写 ADR 10/13 时代的
> `agent/runtime/`（`AgentRuntime` / `AgentScopeAgentRuntime` / `AgentRuntimeProperties`）与
> `agent/llm/`（`LlmModelFactory` 等）旧结构。当前代码中这两处已不存在：`runtime/` 拆除，
> `AgentRuntime` → `agent.AgentExecutor`（接口）+ `agent.AgentScopeReActAgent`（AgentScope ReAct
> 默认实现，含 `agent/tool/` 工具包）；`agent.llm` 上提为顶层 `com.openalice.llm/`
> （`LlmProvider` / `LlmModelFactory` / `LlmSettings` / `ConfiguredHttpTransport`），
> mock（`DeterministicChatModel`）已删除。§2~§7 的对应段落待下次完整重写后再读。
