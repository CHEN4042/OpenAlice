...
> 性质：**速查索引，不是规格**。一切以《项目需求说明书》与 `docs/` 决策为准；权威层级与冲突处理见 `AGENTS.md`。
> 维护：新增术语或决策落地时同步更新；保持精简，别写成第二份需求书。

## 一句话定位
L.E.X.I.N.G.T.O.N.（列克星敦）= 面向**唯一用户本人**的 AI 陪伴助手 —— "一个倾听的、共情的、温柔的、永不忘记你的存在"。情绪陪伴 + 深度交互，**记忆是灵魂**。

## 命名双层体系（工程标识统一用 ① openlexington）
| 层级 | 名称 | 使用场景 |
| :--- | :--- | :--- |
| ① 工程代号 | **openlexington** | GitHub 仓库名（OpenLexington）、工程根目录、工程标识前缀（Maven / 包名 / 变量等，预选） |
| ② 全称 | **L.E.X.I.N.G.T.O.N.**（列克星敦 · Lexington，LEK-sing-tun） | Logo、README 标题、开机问候；AI 自称（中英均可） |

> 全称 = 九特质首字母（L-E-X-I-N-G-T-O-N），"名字即人格说明书"（对应需求书 §0.1）。
...
| M1–M3（记忆） | M1 短期（Redis `openlexington:`）→ M2 结构化（PG）→ M3 语义（PG+pgvector）；**自研掌控** | 需求书 §8 |
...
## 选型结论（以 docs/01 为准，属"技术预选"）
- **Agent 框架**：AgentScope Java 2.0（选型定稿，`docs/01`）——版本策略：**按 2.0.2 实测，遇流式回归回退 2.0.0**。
